...
module.exports=async({root,home,provider,sentinel,eventually,evidence,data})=>{
 ...
 for(const enabled of [true,false]){
  ...
  try{
   ...
   child.stdin.write('\x03');await new Promise(resolve=>setTimeout(resolve,150));
   if(!exited)child.stdin.write('\x03');
   await eventually(async()=>exited,'actual CLI clean shutdown before IDE handoff');
