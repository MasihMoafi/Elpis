...
"#;
    ...
    let req_pos = source
        .find("formatted = format!")
        .expect("find query in source");
...
"#;
    ...
    let enclosing = extractor
        .find_enclosing_scope(&scopes, auth_pos)
        .expect("enclosing scope");
    ...
    assert!(
        enclosing
            .signature
            .contains("pub async fn authenticate(&self, token: &str) -> bool")
    );
    ...
    let struct_scope = extractor
        .find_enclosing_scope(&scopes, struct_pos)
        .expect("struct scope");
...
"#;
    ...
    let enclosing = extractor
        .find_enclosing_scope(&scopes, query_pos)
        .expect("enclosing scope");
    ...
    assert!(
        enclosing
            .signature
            .contains("async def process_records(self, records: list) -> int:")
    );
    ...
    let top_scope = extractor
        .find_enclosing_scope(&scopes, top_func_pos)
        .expect("standalone scope");
...
"#;
    ...
    assert!(
        enclosing
            .signature
            .contains("public async executeQuery<T>(query: string): Promise<T>")
    );

    let arrow_pos = source
        .find("Response: ${JSON.stringify")
        .expect("find in arrow");
    let arrow_scope = extractor
        .find_enclosing_scope(&scopes, arrow_pos)
        .expect("arrow scope");
...
"#;
    ...
    let method_scope = extractor
        .find_enclosing_scope(&scopes, method_pos)
        .expect("method scope");
    ...
    assert!(
        method_scope
            .signature
            .contains("func (s *Service) StartServer() error")
    );

    let func_pos = source.find("sum += uint32(b)").expect("find in func");
    let func_scope = extractor
        .find_enclosing_scope(&scopes, func_pos)
        .expect("func scope");
...
"#;
    ...
    let deploy_scope = extractor
        .find_enclosing_scope(&scopes, deploy_pos)
        .expect("deploy scope");
    ...
    let cleanup_scope = extractor
        .find_enclosing_scope(&scopes, cleanup_pos)
        .expect("cleanup scope");
...
"#;
    ...
    let scope = extractor
        .find_enclosing_scope(&scopes, pos)
        .expect("cpp scope");
