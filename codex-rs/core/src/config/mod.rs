...
pub struct Config {
    ...
    pub animations: bool,
    pub tui_appearance: codex_config::types::TuiAppearance,
    ...
    /// Directory holding the durable `MEMORY.md` note and per-workspace
    /// continuity context.
    pub memory_dir: AbsolutePathBuf,
...
impl Config {
    ...
    async fn load_config_with_layer_stack_and_options(
        ...
        helper_readable_roots.push(memory_dir.clone());
        ...
        let config = Self {
            ...
            tui_appearance: cfg.tui.as_ref().map(|t| t.appearance).unwrap_or_default(),
