...
use codex_prompts::CLEANUP_PROMPT;
use codex_prompts::CONTEXT_PRUNE_PROMPT;
...
struct CompactPreservationScenario {
    next_main_request: String,
    rollout: String,
    request_bodies: Vec<String>,
}

async fn run_compact_preservation_scenario(compact: bool) -> CompactPreservationScenario {
...
async fn compact_preservation_control_without_compaction_keeps_all_markers() {
    ...
    let scenario = run_compact_preservation_scenario(false).await;
    for marker in [
        KEEP_USER_DECISION,
        KEEP_ASSISTANT_RULE,
        ...
        assert!(
            body_contains_text(&scenario.next_main_request, marker),
            "control request should retain {marker}"
        );
        assert!(
            scenario.rollout.contains(marker),
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn native_compact_uses_summarization_without_elpis_cleanup_route() {
    skip_if_no_network!();

    let scenario = run_compact_preservation_scenario(true).await;
    let requests = &scenario.request_bodies;
    let expected_native_compact_request_count = 5;
    assert!(
        requests
            .iter()
            .all(|body| !body_contains_text(body, CONTEXT_PRUNE_PROMPT)),
        "native compact must not issue an Ace-prune request"
    );
    assert_eq!(requests.len(), expected_native_compact_request_count);
    let compact_body = &requests[requests.len() - 2];
    assert!(
        body_contains_text(compact_body, SUMMARIZATION_PROMPT),
        "native compact must use the summarization prompt"
    );
    assert!(
        !body_contains_text(compact_body, CLEANUP_PROMPT),
        "native compact must not use the Elpis cleanup prompt"
    );
}
