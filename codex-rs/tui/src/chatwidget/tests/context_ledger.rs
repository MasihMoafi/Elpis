...
async fn transcript_and_ledger_stay_readable_with_animations_enabled() {
    ...
    assert_eq!(
        ...
        plain
            ...
            .map(|cell| cell.symbol())
            .collect::<Vec<_>>(),
        "animation must never remove readable glyphs"
    );
    assert_ne!(
        animated, plain,
        "streaming reveal must still animate brightness"
    );
...
async fn ledger_groups_real_sources_and_exposes_selected_reason() -> anyhow::Result<()> {
    ...
    assert!(unfocused.contains("Tab controls"));
