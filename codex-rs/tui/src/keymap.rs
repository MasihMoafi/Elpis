...
Set unique keys in `~/.elpis/config.toml` and retry. \
...
Set a different key in `~/.elpis/config.toml` and retry. \
See the Codex keymap documentation for supported actions and examples."
                ));
            }
        }
...
const MAIN_RESERVED_BINDINGS: &[(&str, KeyBinding)] = &[
    ...
    ("fixed.cycle_approval_preset", key_hint::shift(KeyCode::Tab)),
