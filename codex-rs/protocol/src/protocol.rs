...
pub struct ThreadSettingsOverrides {
    ...
    /// Updated model provider id (a key of `model_providers`). When set, it wins
    /// over the provider that would otherwise be inferred from the model slug,
    /// which is the only way to reach providers whose model names carry no
    /// recognizable prefix (locally served Ollama models, for example).
