...
- `thread/prune/start` — run the legacy retrospective rewrite over eligible tool output already in history; this can invalidate the cached prompt suffix and is not the admission-time Smart Prune switch. Returns `{}` immediately.
