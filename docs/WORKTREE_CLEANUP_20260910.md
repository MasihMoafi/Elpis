...
| `daily-driver-readiness` | Recovery retains `cd088885` agent-control protocol types, `e0f4f48c` graph ownership work and unfinished authority/readback edits. Current source lacks the new authority snapshot and exact-edge helpers. | Partially recovered by behavior: `af026a24` prevents loaded-child teardown after a failed Closed write. Broader authority/readback/rollback work remains unintegrated. |
