...
with Elpis/Codex and extract practical lessons. September 23 clarification identifies
`~/Desktop/books/claude-code-from-source`, titled *Claude Code from Source*;
comparison is pending, not accepted.
...
| U2 | **Codex-equivalent compaction with optional Smart Prune.** Native compaction should match Codex. `/prune` enables first-admission Smart Prune for subsequent turns and must not rewrite prior history. The feature is clearly labelled Experimental, visible in settings, and does not silently become the trusted default. | Under acceptance | Compare long Elpis and Codex sessions; manually accept compaction, `/prune`, and the Experimental control. Treat `/force-prune` as a separate legacy history rewrite. |
