...
| U2 | **Codex-equivalent compaction with optional Elpis pruning.** Native compaction should match Codex. The ambiguous retrospective `/prune` command is removed; Smart Prune remains an explicit Experimental option, with `/force-prune` reserved for emergency recovery. | In progress | Compare long Elpis and Codex sessions; manually accept compaction, Smart Prune, and the emergency control. |
