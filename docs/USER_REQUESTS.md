...
| U2 | **Codex-equivalent compaction with optional Elpis pruning.** Native compaction should match Codex. `/prune` enables default-off Smart Prune for subsequent turns; it does not rewrite sent history. `/force-prune` is reserved for explicit emergency recovery. | In progress | Compare long Elpis and Codex sessions; manually accept compaction, `/prune` enabling Smart Prune, and the emergency control. |
| U3 | **A deliberately tiny, user-chosen skill/context set.** Development-rule sources are selected through local configuration; personal paths are not part of this public ledger. Unrelated bundled/installed skills must not flood the Context Ledger. The only currently hand-picked skill is `experiment-workflow`; `first-principles` was explicitly removed because its always-on process was distracting. No additional skill becomes an Elpis default without Masih choosing it. Ledger provenance and token estimates must describe the actual source files. | Under acceptance | Inspect the Ledger and settings; approve the exact final allowlist and verify no unrelated skills are loaded by default. |
...
| U5 | **A genuinely useful, visually strong dashboard.** `/dashboard` should prioritize actionable session/context/agent information, strong UX, and a next-level Elpis-specific visual design—not a cosmetic data dump. | In progress | Manually review the rendered dashboard and accept both usefulness and appearance. |
...
| U10 | **One integrated, releasable Linux build.** Reconcile the stable Smart Prune, context, dashboard, memory, skills/plugins, provider, and work-graph work; require an exact-head green Linux release gate; publish and install only that verified artifact; prove its hashes match. | Release authorized once the gate passes | Confirm the published artifact, installed binary, checksums, and manual daily-driver behavior. |
...
On 2026-09-05 Masih accepted the installed v0.2.0 copper candidate ("it's perfect")
and authorized its privacy-reviewed release, followed by paper, public repository,
site SEO, and an X announcement. This accepts the tested candidate's visible
behavior; it does not mark every broader outcome or unrun experiment as complete.
...
- Push, tag, hosted release, package publication, and installation are authorized only for the
  exact `v0.2.0` candidate after its documented release gate passes. Worktree deletion and
  unrelated process restarts remain out of scope.
