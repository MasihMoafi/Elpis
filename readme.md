...
Elpis starts from OpenAI's Apache-2.0 Codex CLI and keeps its execution foundation — terminal UI, patches, permissions, sandboxing, and sessions. It extends that foundation with a more explicit context-control layer: selective tool-output pruning with evidence pointers, user-controlled context admission, context-usage inspection, auditable pruning decisions, and portable task checkpoints that survive compaction, restarts, and provider changes.
